<template>
  <form-style action="" class="form" id="form" @submit.prevent="formValidation">
    ...
    />
    <p
        v-if="validateError(v$.faculty.name)===1" class="error"
    >{{ this.errors["empty"] }}</p>
    ...
    <div class="wrapper__input">
      ...
      />
      <p
          v-if="validateError(v$.faculty.shortName)===1" class="error"
      > {{ this.errors["empty"] }}</p>
...
export default {
  ...
  methods: {
    ...
    editForm(){
      this.$emit('editFaculty', this.faculty)
